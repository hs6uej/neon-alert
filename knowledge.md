...
| แท็บ | ทำอะไรได้ |
| --- | --- |
| **Structures / Units** | เห็น**รูป**ของทุกตัว · สวิตช์ **In battle** เปิด/ปิดว่าจะลงสนามรบหรือไม่ · แก้ราคา เวลาสร้าง HP ความเร็ว พลังงาน มองเห็น เกราะ อาวุธ ข้อกำหนด · **สร้างอาคาร/ยูนิตใหม่** (สำเนาของชนิดเดิมพร้อมความสามารถของต้นแบบ) · **🎨 Picture studio**: สร้างรูปของอาคาร/ยูนิตด้วย AI (Google Gemini), อัปโหลดไฟล์ หรือวาดเอง — ตัดพื้นหลังอัตโนมัติ พรีวิวในเกม แล้วใช้แทนภาพที่วาดด้วยโค้ด · **ชุดรูป**: V1 (ภาพวาดด้วยโค้ดเดิม), V2 (AI วาดครบ 22 แบบ แสงนีออนเปลี่ยนเป็นสีทีม), และชุดที่แอดมินสร้าง/ตั้งชื่อเอง (สูงสุด 8) — เลือกชุดที่เกมใช้ได้ที่แท็บ Structures / Units (`artSet` ใน config.json) |
...
| `art/*.png` | รูปอาคาร/ยูนิตที่แอดมินสร้างในชุดของตัวเอง (รูปที่ไม่มีชุดไหนอ้างอิงจะถูกลบเมื่อเก่าเกิน 1 วัน และลบเฉพาะตอนกด Save คอนฟิก ไม่ลบตอน deploy/สตาร์ต; ชุดสำเร็จรูป V2 อยู่ใน `public/sets/v2/` ตามซอร์ส ไม่อยู่ใน data) | ต่ำ |
...
| GET | `/api/admin/art/info` | แอดมิน | บอกว่าเปิด AI วาดรูปอยู่ไหม + ชื่อโมเดล (ไม่คืน key) |
| POST | `/api/admin/art/generate` | แอดมิน | ส่งคำบรรยาย (+ภาพอ้างอิง) ไปให้ Gemini วาด คืนภาพ PNG (พื้นหลังสีแมเจนต้า — เบราว์เซอร์ตัดออกเอง); เรียกพร้อมกันได้ ≤ 2 |
| PUT | `/api/admin/art` | แอดมิน | เก็บ PNG ที่ตัดแล้ว (≤ 800 KB, 8–2048 px; แถบเฟรมเรียงแนวนอนได้ถึง 8 เฟรม) เป็น `data/art/<type>-<version>.png` คืนเลขเวอร์ชัน |
| GET | `/art/<type>-<version>.png` | สาธารณะ | รูปที่ผู้เล่นทุกคนโหลด (cache ถาวร เพราะชื่อไฟล์มีเวอร์ชัน) |
| GET | `/sets/<set>/<type>.png?v=N` | สาธารณะ | รูปของชุดสำเร็จรูป (V2) — cache ถาวร เปลี่ยน `v` ใน `artsets.js` เมื่อเปลี่ยนรูป |
...
| `GEMINI_API_KEY` | — | key ของ Google Gemini สำหรับให้ AI วาดรูปอาคาร/ยูนิต (ไม่ตั้ง = ปิดปุ่ม AI แต่อัปโหลด/วาดเองยังใช้ได้; key อยู่ฝั่งเซิร์ฟเวอร์เท่านั้น; ค่าใช้จ่ายราว 1 คำขอ/รูป) |
| `GEMINI_IMAGE_MODEL` | `gemini-2.5-flash-image` | โมเดลที่ใช้วาด — ต้องเป็นโมเดลที่ออกรูปได้ (โมเดลข้อความอย่าง `*-flash-lite` วาดไม่ได้) |
